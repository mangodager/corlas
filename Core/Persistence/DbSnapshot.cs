...
namespace ETModel 
{
    public class DbSnapshot : IDisposable
    {
        ...
        public DbCache<Block>        Blocks { get; }          // 根据块哈希保存所有块内容
        public DbCache<List<string>> Heights { get; }         // 保存某个高度下 所有块的哈希值
        public DbCache<Transfer>     Transfers { get; }       // 保存所有交易
        public DbCache<Account>      Accounts { get; }        // 保存账户
        public DbCache<LuaVMScript>  Contracts { get; }       // 保存智能合约
        public DbCache<LuaVMContext> Storages { get; }        // 存储
